 ...
 * Namespaces
 */
namespace UniversalPayment\Frontend\ContentElement;
 ...
 */
class ContentPayment extends \ContentElement
{
	/**
	 * Template
	 * @var
	 */
	protected $strTemplate = 'ce_payment';
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$this->Template->wildcard = '### CONTENT UNIVERSAL PAYMENT ###'. '<p class="tl_gray">'.$strMethodName.'</p>';
	...
	protected function compile()
	{
		...
		$objResult = $objDatabase->prepare("SELECT * FROM tl_content WHERE id=?")->limit(1)->execute($this->id);
